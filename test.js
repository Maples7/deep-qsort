...
test('Deep quick sort a flatten number array', async t => {
  t.plan(1);
  const ret = deepSort([4, 3, 25, 3, 9, -1, 0, 24, 2, 13, 11, -9]);
  t.deepEqual(ret, [-9, -1, 0, 2, 3, 3, 4, 9, 11, 13, 24, 25]);
});

test('Deep quick sort a flatten string array', async t => {
  t.plan(1);
  const ret = deepSort(['a', 'd', 'b', 'c']);
  t.deepEqual(ret, ['a', 'b', 'c', 'd']);
});

test('Deep quick sort a deep number array', async t => {
  t.plan(1);
  const ret = deepSort([4, 3, [25, 3, [9], [-1, 0], 24], 2, 13, 11, -9]);
